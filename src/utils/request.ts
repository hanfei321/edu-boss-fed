...
function refreshToken() {
  return axios.create()({
    ...
    data: qs.stringify({
      ...
      refreshtoken: Login.state.userLoginData.refresh_token
    })
  })
}
request.interceptors.request.use(function (config: AXIrequestconfig) {
  // 我们就在这里通过改写 config 配置信息来实现业务功能的统一处理
  const user = Login.state.userLoginData
